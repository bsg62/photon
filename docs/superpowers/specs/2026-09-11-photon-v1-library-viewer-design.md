...
- Formats: JPEG, PNG, GIF, WebP, HEIC, AVIF; video (poster-frame thumbnails and playback).
- Smooth operation up to about 100,000 items.

### Out of scope (v1)

Edits, albums, tags, ratings, captions, search, faces, maps, XMP sidecar reading/writing, RAW formats, export/upload, slideshows. The database schema must not preclude them, but no code or tables are added for them now.
...
- **Native libraries:** libheif (HEIC/AVIF decoding), ffmpeg (video poster frames and metadata). Both are LGPL, dynamically linked, bundled with the app on macOS and Windows, and taken from system packages on Linux.
...
| `metadata` | Extracts capture date, dimensions, orientation from images (EXIF) and videos (ffmpeg). Pure function of a file path. | kamadak-exif, ffmpeg |
| `decode` | Turns a file into an oriented RGBA bitmap at a requested maximum size. Picks pure-Rust decoders, libheif or ffmpeg by format. Reports unsupported formats when a native library failed to load. | image, libheif, ffmpeg |
...
- `photon://image/<id>`: for JPEG/PNG/GIF/WebP, streams the original file with its correct MIME type. For HEIC/AVIF, converts to high-quality JPEG in Rust (cached in the preview cache tier at full resolution for the session).
- `photon://video/<id>`: streams the original video file with HTTP range support so `<video>` can seek.
...
- EXIF orientation is applied in Rust for all generated images (thumbnails, previews, HEIC/AVIF conversions), and through CSS `image-orientation: from-image` for streamed originals.
- Video: `<video>` pointed at `photon://video/<id>`. If the webview reports the codec as unsupported (for example HEVC on Linux WebKitGTK), show the poster frame plus an "Open in system player" button.
...
- **Missing native library** (libheif or ffmpeg fails to load): those formats are reported as unsupported (a notice in the UI, and per-item failed state). Everything else keeps working.
...
- **Decoder fixtures:** a small committed set covering JPEG in all 8 EXIF orientations, PNG, GIF, WebP, HEIC, AVIF, a short MP4, and deliberately corrupt files.
...
- **CI:** GitHub Actions matrix on Linux, macOS and Windows that builds, runs all tests, and produces installable packages (AppImage/deb, dmg, msi) with the native libraries bundled where required.
...
- Every listed format displays on all three operating systems.
