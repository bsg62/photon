...
- [ ] Export into a folder that photon watches: it is refused with a message naming that folder, and nothing is written. Export a second time into a folder that already holds a copy of the same name: the new file arrives as `name (2).jpg` and the first one is untouched.
