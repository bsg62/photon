...
describe('yearMarks', () => {
  ...
  it('marks a year again when it comes back, as a search can make it', () => {
    ...
    expect(yearMarks(sections, buildRows(sections, 4, TILE_WIDTH.medium)).map((m) => m.year)).toEqual([2020, 2024, 2020]);
  });

  it('has nothing to mark without headers', () => {
    const sections = [{ ...section(1, 0, 3, 2024), folderId: null }];
    expect(yearMarks(sections, buildRows(sections, 2, TILE_WIDTH.medium))).toEqual([]);
  });
});
